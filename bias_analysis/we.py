...
class WordEmbedding:
    def __init__(self, fname):
        ...
        elif fname.endswith(".kv"):
            model = KeyedVectors.load(fname)  # load .kv
            words = sorted(model.key_to_index.keys(), key=lambda w: model.key_to_index[w])
            vecs = [model[w] for w in words]
...
def doPCA(pairs, embeddings, num_components=10):
    """
    pairs: list of [word_a, word_b]
    embeddings: gensim KeyedVectors
    """
    matrix = []

    for a, b in pairs:
        # Skip pairs missing from the vocabulary
        ...
        matrix.append(va - center)
        matrix.append(vb - center)
